...
Application.prototype._render = function(filterText = '') {
    this.destroyTable();
    this.actorsList.filter(item => !filterText || (filterText && item.actorName.includes(filterText))).forEach(actor => {
...
Application.prototype.onActorsLoaded = function(actors) {
    actors.forEach(({ id, characterName, actorName, pictureURL, yearOfBirth = 'Not provided' }) => {
        var actor =  new Actor(id, characterName, actorName, pictureURL, yearOfBirth);
